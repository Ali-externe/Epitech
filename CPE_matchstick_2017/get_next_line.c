...
{
	static char *more; // * sur le buffer que je ++ a chaque apel de fction
	static int equal; // = au read que je décremente a chaque fois
	char c;
	...
	c = *more; // c = l'endroit où je pointe sur le buffer
	equal--;
	more = more + 1;
	return (c);
}
